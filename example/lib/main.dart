...
void main() {
  runApp(const IldsPlaygroundApp());
}

class IldsPlaygroundApp extends StatelessWidget {
  const IldsPlaygroundApp({super.key});

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      debugShowCheckedModeBanner: false,
      title: 'ILDS Playground',
      theme: ThemeData(useMaterial3: true),
      home: const IldsPlaygroundPage(),
    );
  }
}

class IldsPlaygroundPage extends StatefulWidget {
  const IldsPlaygroundPage({super.key});

  @override
  State<IldsPlaygroundPage> createState() => _IldsPlaygroundPageState();
}

class _IldsPlaygroundPageState extends State<IldsPlaygroundPage> {
  int selectedNav = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('ILDS Component Playground'),
          ...
          child: Padding(
            ...
            child: Row(
              children: const [
                ...
                SizedBox(width: 8),
                Text(
                  'Live linked to repo source via path dependency (hot reload reflects updates).',
